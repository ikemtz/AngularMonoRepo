...
export function createKendoODataGridSelector<
  TEntity,
  TPartialState,
  TState extends KendoODataGridState<TEntity>
>(
  ...
  getGridData: MemoizedSelector<
    object,
    ODataResult<TEntity>,
    DefaultProjectorFn<ODataResult<TEntity>>
  >;
  getGridODataState: MemoizedSelector<
    object,
    ODataState,
    DefaultProjectorFn<ODataState>
  >;
  getPagerSettings: MemoizedSelector<
    object,
    false | PagerSettings,
    DefaultProjectorFn<false | PagerSettings>
  >;
} {
  const entireStateSelector = createFeatureSelector<TState>(
  ...
  return {
    ...
    getPagerSettings: createSelector(
      ...
      getGridODataState,
      (result, state) =>
        getODataPagerSettings<TEntity>({
          gridData: result,
          gridODataState: state,
        })
    ),
  };
}
