...
export const mapToExtDataResult = <T extends Record<string, unknown>>(
...
export const firstRecord = <T>() =>
  map((result: ODataResult<T>) =>
    result?.data?.length > 0 ? result.data[0] : ({} as T),
  );

export const findById = <T extends { id?: IdType }>(
  id?: IdType,
  defaultValue: T = {} as T,
) =>
  map(
    (source: ODataResult<T>) =>
      source?.data?.find((f) => f.id === id) || defaultValue,
  );

export function parseDatesInCollection<T extends Record<string, unknown>>(
  collection: Array<T>,
  utcNullableProps: string[] = [],
  dateNullableProps: string[] = [],
): Array<T> {
