...
export class SaleOrderEffects {
  ...
  loadSaleOrdersEffect$ = createEffect(() =>
    this.dataPersistence.fetch(saleOrderActionTypes.loadSaleOrdersRequest, {
      run: (action: ReturnType<typeof saleOrderActionTypes.loadSaleOrdersRequest>) =>
        this.odataservice
          .fetch<ISaleOrder>(environment.endPoints.sale_orders, action.payload)
          .pipe(
            map(t => saleOrderActionTypes.loadSaleOrdersSuccess(t)),
          ),
  ...
  saveSaleOrderEffect$ = createEffect(() =>
    this.dataPersistence.pessimisticUpdate(saleOrderActionTypes.saveSaleOrderRequest, {
      run: (action: ReturnType<typeof saleOrderActionTypes.saveSaleOrderRequest>, partialState: fromSaleOrdersReducer.SaleOrdersPartialState) =>
        this.saleOrderApiService.post(action.payload).pipe(
          map(() => saleOrderActionTypes.loadSaleOrdersRequest(partialState[fromSaleOrdersReducer.SALE_ORDERS_FEATURE_KEY].gridODataState))),
      onError: this.exceptionHandler,
    })
  );
  ...
  updateSaleOrderEffect$ = createEffect(() =>
    this.dataPersistence.pessimisticUpdate(saleOrderActionTypes.updateSaleOrderRequest, {
      run: (action: ReturnType<typeof saleOrderActionTypes.updateSaleOrderRequest>, partialState: fromSaleOrdersReducer.SaleOrdersPartialState) =>
        this.saleOrderApiService.put(action.payload).pipe(
          map(() => saleOrderActionTypes.loadSaleOrdersRequest(partialState[fromSaleOrdersReducer.SALE_ORDERS_FEATURE_KEY].gridODataState))),
      onError: this.exceptionHandler,
    })
  );
  ...
  deleteSaleOrderEffect$ = createEffect(() =>
    this.dataPersistence.pessimisticUpdate(saleOrderActionTypes.deleteSaleOrderRequest, {
      run: (action: ReturnType<typeof saleOrderActionTypes.deleteSaleOrderRequest>, partialState: fromSaleOrdersReducer.SaleOrdersPartialState) =>
        this.saleOrderApiService.delete(action.payload).pipe(
          map(() => saleOrderActionTypes.loadSaleOrdersRequest(partialState[fromSaleOrdersReducer.SALE_ORDERS_FEATURE_KEY].gridODataState))),
      onError: this.exceptionHandler,
    })
  );
