  ...
  loadProductsEffect$ = createEffect(() =>
    this.dataPersistence.fetch(productActionTypes.loadProductsRequest, {
      run: (action: ReturnType<typeof productActionTypes.loadProductsRequest>) =>
        this.odataservice
          .fetch<IProduct>(environment.endPoints.products, action.payload, [], [ProductProperties.SELL_END_DATE])
          .pipe(map((t) => productActionTypes.loadProductsSuccess(t))),
  ...
  saveProductEffect$ = createEffect(() =>
    this.dataPersistence.pessimisticUpdate(productActionTypes.saveProductRequest, {
      run: (
        ...
        partialState: fromProductsReducer.ProductsPartialState,
      ) =>
        this.productApiService
          .post(action.payload)
          .pipe(
            map(() =>
              productActionTypes.loadProductsRequest(
                partialState[fromProductsReducer.PRODUCTS_FEATURE_KEY].gridODataState,
              ),
            ),
          ),
      onError: this.exceptionHandler,
