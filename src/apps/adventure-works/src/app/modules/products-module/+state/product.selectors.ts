...
import { PRODUCTS_FEATURE_KEY, State } from './product.reducer';
import { isTruthy } from 'imng-ngrx-utils';

export const getProductsState = createFeatureSelector<State>(PRODUCTS_FEATURE_KEY);
...
const getCurrentProduct = createSelector(getProductsState, (state) => state.currentProduct);
const getIsEditProductActive = createSelector(getCurrentProduct, (entity) => isTruthy(entity) && isTruthy(entity?.id));
const getIsNewProductActive = createSelector(getCurrentProduct, (entity) => isTruthy(entity) && !isTruthy(entity?.id));
