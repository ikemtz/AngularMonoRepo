...
export class BuildingEffects {
  ...
  loadBuildingsEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((action: ReturnType<typeof buildingActionTypes.loadBuildingsRequest>) => this.odataservice
  ...
  reloadBuildingsEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      concatLatestFrom(() => this.store.select(buildingsFeature.selectGridODataState)),
      switchMap(([action, odataState]) => this.odataservice
        .fetch<IBuilding>(environment.endPoints.buildings.buildingsOData, odataState, {
          bustCache: true,
          dateNullableProps: [BuildingProperties.DELETED_ON_UTC],
        })
        .pipe(
  ...
  saveBuildingEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((action: ReturnType<typeof buildingActionTypes.saveBuildingRequest>) => this.buildingApiService.post(action.payload).pipe(
  ...
  updateBuildingEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((action: ReturnType<typeof buildingActionTypes.updateBuildingRequest>) => this.buildingApiService.put(action.payload).pipe(
        map(() => buildingActionTypes.reloadBuildingsRequest()),
        handleEffectError(action))));
  });
  ...
  deleteBuildingEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((action: ReturnType<typeof buildingActionTypes.deleteBuildingRequest>) => this.buildingApiService.delete(action.payload).pipe(
        map(() => buildingActionTypes.reloadBuildingsRequest()),
        handleEffectError(action))));
  });
}
