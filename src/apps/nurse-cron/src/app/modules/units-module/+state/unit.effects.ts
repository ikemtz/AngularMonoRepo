...
export class UnitEffects {
  ...
  loadUnitsEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((action: ReturnType<typeof unitActionTypes.loadUnitsRequest>) => this.odataservice
  ...
  reloadUnitsEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      concatLatestFrom(() => this.store.select(unitsFeature.selectGridODataState)),
  ...
  saveUnitEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((action: ReturnType<typeof unitActionTypes.saveUnitRequest>) => this.unitApiService.post(action.payload).pipe(
  ...
  updateUnitEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((action: ReturnType<typeof unitActionTypes.updateUnitRequest>) => this.unitApiService.put(action.payload).pipe(
        map(() => unitActionTypes.reloadUnitsRequest()),
        handleEffectError(action))));
  });
  ...
  deleteUnitEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((action: ReturnType<typeof unitActionTypes.deleteUnitRequest>) => this.unitApiService.delete(action.payload).pipe(
        map(() => unitActionTypes.reloadUnitsRequest()),
        handleEffectError(action))));
  });
  ...
  loadBuildingsEffect$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((action: ReturnType<typeof unitActionTypes.loadBuildingsRequest>) => this.odataservice
        .fetch<IBuilding>(environment.endPoints.buildings.buildingsOData, action.payload)
        .pipe(map(t => unitActionTypes.loadBuildingsSuccess(t)),
          handleEffectError(action))));
  });
}
