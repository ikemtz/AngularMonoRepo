...
export function translateChildFilterExpression(
  ...
  const filterPredicate = (filter: FilterDescriptor | SortDescriptor | any) => { //NOSONAR
    return filter.field === childFieldString;
  };
  const childTableFilter: CompositeFilterDescriptor = odataState.filter?.filters.find((t: CompositeFilterDescriptor | FilterDescriptor) =>
  ...
  if (childTableFilter && odataState.filter) {
    odataState.filter = {
      ...odataState.filter,
      filters: [
        ...odataState.filter.filters.filter((t: FilterDescriptor | CompositeFilterDescriptor) => isCompositeFilterDescriptor(t) && !t.filters?.some(filterPredicate)),
      ],
    };
    odataState.childFilters = {
      ...
      filters:
        odataState.childFilters?.filters.filter(
          (childFilterDescriptor: ChildFilterDescriptor | CompositeChildFilterDescriptor) =>
    ...
    odataState.childFilters.filters.push({
      ...
      filters: childTableFilter?.filters?.map((filter: FilterDescriptor | CompositeFilterDescriptor) => ({
