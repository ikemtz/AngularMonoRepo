/**
 * This file is generated by the odata-ts-generator
 * For issues or feature request, visit the repo: https://github.com/ikemtz/OData-TS-Generator
 * Do not edit.
 */
 
export * from './certification-insert-request.model';
export * from './certification.model';
export * from './certification-update-request.model';
export * from './certification-insert-request.form-group-fac';
export * from './certification.form-group-fac';
export * from './certification-update-request.form-group-fac';
